const API_URL = 'https://impartial-vagabond-psychology.glitch.me/';
...
const initService = () => {
  ...
  renderFieldsetService.innerHTML = '<legend class="reserve__legend">Услуга</legend>';
  addPreload(renderFieldsetService);
  ...
  .then(data => {
    renderService(renderFieldsetService, data);
    removePreload(renderFieldsetService);
  })
};
...
const renderTime = (wrapper, data) => {
  const labels = data.map(time => {
    ...
    label.classList.add('radio');
    label.innerHTML = `
    <input class="radio__input" type="radio" name="day" value="${time}" />
